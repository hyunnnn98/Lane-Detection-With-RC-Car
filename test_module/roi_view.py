...
def draw_lines(img, lines, color=BLUE_COLOR, thickness=2):
    for line in lines:
...
def weighted_img(img, intial_img, α=1, β=1., λ=0.):
    return cv2.addWeighted(intial_img, α, img, β, λ)


image = cv2.imread('y_playground.png')
...
canny_img = canny(blur_img, 35, 105)
